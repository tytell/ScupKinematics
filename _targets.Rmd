...
outeven <-
  d1$outline[[1]] |> 
  get_midline(xctr,yctr,side, spar = 0.3, debug = FALSE)

outeven

outeven |> 
  plot_ly() |> 
  add_markers(x = ~xL0, y = ~yL0, name = "L0") |> 
  add_markers(x = ~xR0, y = ~yR0, name = "R0") |> 
  add_lines(x = ~xL, y = ~yL, name = "L") |> 
  add_lines(x = ~xR, y = ~yR, name = "R") |> 
  add_lines(x = ~xmid, y = ~ymid, name = "mid") |> 
  layout(yaxis = list(
    scaleanchor = "x",
    scaleratio = 1))
...
sps <-
  ...
  duplicate_head_tail() |> 
  group_by(side) |> 
  arrange(desc(xctr), .by_group = TRUE) |> 
  mutate(s = seq(0, 1, by = 1/(n()-1))) |> 
  ungroup() |>   
  pivot_wider(values_from = c(xctr, yctr),
              names_from = side) |> 
  summarize(across(c(xctr_L, xctr_R, yctr_L, yctr_R),
                   \(y) list(stats::smooth.spline(x = s, y = y,
                                             spar = 0.3, cv = NA))))
```
...
sps |> 
  reframe(across(dplyr::everything(), 
                 \(sp) predict(sp[[1]], x = s1)$y,
                 .names = "{.col}_s"))
```
...
```{r}
d3 <- d2 |> 
  select(imnum, camera, xctr,yctr, point, side) |> 
  group_by(imnum, camera) |> 
  group_modify(duplicate_head_tail)
```
...
d4 <-
  ...
  dplyr::arrange(desc(xctr), .by_group = TRUE) |> 
  mutate(ds = sqrt((xctr - lag(xctr))^2 + (yctr - lag(yctr))^2)) |> 
  group_by(point) |> 
  mutate(ds_med = median(ds, na.rm = TRUE),
         ds_iqr = IQR(ds, na.rm = TRUE),
         isoutlier = abs(ds - ds_med) > 2*ds_iqr)

d4 |> 
  filter(imnum == 2160, camera == "C1", side == "R") |> 
  group_by(imnum, camera, side) |> 
  plot_ly(x = ~point, y = ~ds, color = ~isoutlier) |> 
  add_markers() |> 
  add_lines(y = ~ds_med, color = ~side, name = "median")
```

```{r}
d4s <-
  d4 |> 
  filter(imnum == 2160, camera == "C1") |> 
  smooth_outline(xctr, yctr, side, spar = 0.3) |> 
  group_by(s) |> 
  mutate(xmid = mean(xctr_s),
         ymid = mean(yctr_s)) |> 
  ungroup()
```
...
d4 |> 
  filter(imnum == 2160, camera == "C1") |> 
  plot_ly(x = ~xctr, y = ~yctr, color = ~side) |> 
  add_markers() |> 
  add_lines(data = d4s, x = ~xctr_s, y = ~yctr_s, color = ~side) |> 
  add_lines(data = d4s, x = ~xmid, y = ~ymid) |> 
  layout(yaxis = list(
                      scaleanchor = "x",
                      scaleratio = 1))
```
...
d5 <-
  d4 |> 
  group_by(camera, imnum) |> 
  group_modify(\(df, k) smooth_outline(df, xctr,yctr, side, spar = 0.3)) |> 
  group_by(camera, imnum, s) |> 
  mutate(xmid = mean(xctr_s),
         ymid = mean(yctr_s)) |> 
  ungroup()
...
d5 |> 
  filter(camera == "C1") |> 
  group_by(imnum, side) |> 
  plot_ly(x = ~xctr_s, y = ~yctr_s) |> 
  add_lines() |> 
  add_lines(x = ~xmid, y = ~ymid) |> 
  layout(yaxis = list(
                      scaleanchor = "x",
                      scaleratio = 1))
```
...
```{r}
sp <- with(df1,
           stats::smooth.spline(yctr, spar = 0.5, cv = TRUE))
...
spline_ocv <- function(df, x,y, p)
{
  ...
  sp <- with(df1,
             stats::smooth.spline(x, y, spar = p))
  xy <- predict(sp)
  df1$ys <- xy$y
  ...
  for (i in seq(2,nrow(df)-1)) {  
    k <- c(seq(1,i-1), seq(i+1,nrow(df)))
    sp1 <- with(df1[k,],
               stats::smooth.spline(x, y, spar = p))
    xy1 <- predict(sp1, x = df1$x[i])
    
    df1$yloo[i] <- xy1$y
    df1$ycv[i] <- xy1$y - df1$y[i] 
  }
...
```{r}
df1 |> 
  spline_ocv(xctr, yctr, 0.5) |> 
  ungroup() |> 
  plot_ly(x = ~xctr) |>
  add_markers(y = ~yctr) |> 
  add_markers(y = ~yctr_loo) |> 
  add_lines(y = ~yctr_s)
```
