...
```{r}
d3 <- d2 |> 
  select(imnum, xctr,yctr, x.head,x.tail, y.head,y.tail, point, side) |> 
  pivot_wider(values_from = c(xctr, yctr), names_from = "side") |> 
  select(-ends_with("_T"), -ends_with("_H")) |> 
  mutate(point = as.numeric(point)) |> 
  group_by(imnum) |> 
  group_modify(add_head_tail_points)
```
