...
data1ptsdir <-
  ...
  mutate(dist = sqrt((x - x.head)^2 + (y - y.head)^2),
         len = sqrt((x.head - x.tail)^2 + (y.head - y.tail)^2),
         xctr = x - x.head,
         yctr = y - y.head) |> 
  select(-contains("med")) |> 
  nest(outline = c(xctr, yctr, side, dist, len),
       .by = c(tsec, imnum, speedHz, tempC, camera, x.head, x.tail, y.head, y.tail)) |> 
  mutate(swimdir =
           purrr::map(outline, \(df) get_central_axis(df, x = xctr, y = yctr,
                                                   dist = dist, len = len))) |>
  unnest(swimdir) |> 
  mutate(swimdiry = if_else(swimdirx > 0, -swimdiry, swimdiry),
         swimdirx = if_else(swimdirx > 0, -swimdirx, swimdirx))
